...
由于原仓中采用了多项优化器相关的显存优化措施（例如：使用bitsandbytes的节省内存优化器、CPUOffloadOptimizer、torchao量化和低位内存优化器），而这些措施在MindSpore平台暂无平滑的等价实现，因此我们暂时不支持相关功能，因此不进行对标原仓的内存分析数据。


## 与原仓的差异&功能限制

### 训练脚本结构性差异
为适配MindSpore平台特性，我们调整了原有训练脚本的代码结构。如果您有意了解更深的代码细节，可参见[这篇文档](https://gist.github.com/townwish4git/3a181a1884747dfbbe4b31107ec02166)。

### 🤗Accelerate
原始训练脚本通过调用`accelerate`库提供简单快速的分布式训练、梯度累积、梯度裁剪、Deep-Speed ZeRO并行、混合精度训练等训练相关功能的支持，并通过训练脚本拉起时的`--config_file ACCELERATE_CONFIG_FILE`配置文件进行相关功能的配置。

对于🤗Accelerate，我们暂无其等价实现。对于上述功能我们在脚本中调用不同接口分别进行支持。部分功能参数配置参见[MindSpore特性](#mindspore特性)。
...
### 功能限制
训练脚本部分参数接口存在限制，其中一个主要的限制来自于CogVideoX模型中的3D Causual VAE**不支持静态图**（参考[这篇文档](https://gist.github.com/townwish4git/b6cd0d213b396eaedfb69b3abcd742da)）

具体而言，这些限制包括：

- **不支持静态图模式下VAE参与训练**。在静态图模式下必须提前进行数据预处理以获取VAE-latents/text-encoder-embeddings cache；
- 不支持prodigy和came优化器、CPUOffload优化器、量化和低位内存优化器；
- 暂不支持动态分辨率/帧率训练；
- 不支持`args.enable_model_cpu_offload`;
- 不支持`args.pin_memory`;
- 不支持`args.push_to_hub`;

> [!WARNING]
> 另外在当前的软件版本下，动态图训练存在缓慢的内存泄漏，较长步数训练情况下会导致显存占用异常进而产生OOM Error。因此当前版本并不推荐动态图训练。
