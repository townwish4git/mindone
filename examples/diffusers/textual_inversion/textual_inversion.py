...
def log_validation(pipeline, args, trackers, logging_dir, global_step):
    logger.info(
    ...
    if is_master(args):
        validation_logging_dir = os.path.join(logging_dir, "validation", f"step{global_step}")
    ...
    for tracker_name, tracker_writer in trackers.items():
        if tracker_name == "tensorboard":
            ...
            tracker_writer.add_images("validation", np_images, global_step, dataformats="NHWC")
        else:
...
def main():
    ...
    token_embeds = text_encoder.get_input_embeddings().embedding_table.data
    ...
    freeze_params(text_encoder.text_model.encoder)
    freeze_params(text_encoder.text_model.final_layer_norm)
    freeze_params(text_encoder.text_model.embeddings.position_embedding)
    ...
    optimizer = nn.AdamWeightDecay(
        text_encoder.get_input_embeddings().trainable_params(),  # only optimize the embeddings
    ...
    orig_embeds_params = text_encoder.get_input_embeddings().embedding_table.data.copy()
...
class TrainStepForTI(TrainStep):
    ...
    def forward(self, pixel_values, input_ids):
        ...
        encoder_hidden_states = self.text_encoder(input_ids)[0].to(dtype=self.weight_dtype)

        # Predict the noise residual
        model_pred = self.unet(noisy_latents, timesteps, encoder_hidden_states)[0]
