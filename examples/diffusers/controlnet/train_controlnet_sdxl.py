...
def collate_fn(examples):
    ...
    add_text_embeds = np.stack([example["text_embeds"] for example in examples]).astype(np.float32)
    add_time_ids = np.stack([example["time_ids"] for example in examples]).astype(np.float32)

    return pixel_values, conditioning_pixel_values, prompt_ids, add_text_embeds, add_time_ids
...
def main():
    ...
    def compute_embeddings(batch, proportion_empty_prompts, text_encoders, tokenizers, is_train=True):
        ...
        add_time_ids = ms.Tensor([add_time_ids])
        add_time_ids = add_time_ids.tile((len(prompt_batch), 1))
        add_time_ids = add_time_ids.to(dtype=prompt_embeds.dtype)
        unet_added_cond_kwargs = {"text_embeds": add_text_embeds.numpy(), "time_ids": add_time_ids.numpy()}

        return {"prompt_embeds": prompt_embeds.numpy(), **unet_added_cond_kwargs}
...
class TrainStepForControlNet(TrainStep):
    ...
    def forward(self, pixel_values, conditioning_pixel_values, prompt_ids, add_text_embeds, add_time_ids):
        ...
        unet_added_conditions = {
            "text_embeds": add_text_embeds.to(self.weight_dtype),
            "time_ids": add_time_ids.to(self.weight_dtype),
        }
