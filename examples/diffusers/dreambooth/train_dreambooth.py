...
def log_validation(
    ...
    if args.validation_images is None:
        for _ in range(args.num_validation_images):
            image = pipeline(**pipeline_args, num_inference_steps=25, generator=generator)[0][0]
            images.append(image)
    else:
...
def parse_args(input_args=None):
    ...
    assert args.set_grads_to_none is False, error_template("Setting grads to None", "set_grads_to_none")
...
def main():
    ...
    # Optimizer creation
    params_to_optimize = (
        itertools.chain(unet.trainable_params(), text_encoder.trainable_params())
        if args.train_text_encoder
        else unet.trainable_params()
    )
    ...
    module = importlib.import_module("mindone.diffusers")
