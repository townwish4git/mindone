...
def main():
    ...
    if amp_dtype:
        transformer = auto_mixed_precision(transformer, amp_level=args.amp_level, dtype=amp_dtype)
        if args.train_text_encoder:
            text_encoder_one = auto_mixed_precision(text_encoder_one, amp_level=args.amp_level, dtype=amp_dtype)
            text_encoder_two = auto_mixed_precision(text_encoder_two, amp_level=args.amp_level, dtype=amp_dtype)
            text_encoder_three = auto_mixed_precision(text_encoder_three, amp_level=args.amp_level, dtype=amp_dtype)
...
class SD3DBNetworkWithLoss(nn.Cell):
    ...
    def construct(self, pixel_values, tokens_one, tokens_two, tokens_three):
        ...
        with pynative_no_grad():
            ...
            model_input = (model_input - self.vae_scaling_factor) * self.vae_scaling_factor
