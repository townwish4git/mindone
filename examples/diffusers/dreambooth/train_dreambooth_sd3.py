...
def _encode_prompt_with_t5(text_encoder, text_input_ids, num_images_per_prompt=1):
    batch_size = text_input_ids.shape[0]
    prompt_embeds = text_encoder(text_input_ids)[0]

    # FIXME: pay attention here, ModelMixin.dtype will be called in Cell.construct()
...
def _encode_prompt_with_clip(text_encoder, text_input_ids, num_images_per_prompt: int = 1):
    batch_size = text_input_ids.shape[0]
...
def encode_prompt(
    ...
    clip_text_input_ids = all_text_input_ids[:2]

    clip_prompt_embeds_list = []
    clip_pooled_prompt_embeds_list = []
    for text_encoder, text_input_ids in zip(clip_text_encoders, clip_text_input_ids):
    ...
    clip_prompt_embeds = ops.cat(clip_prompt_embeds_list, axis=-1)
    pooled_prompt_embeds = ops.cat(clip_pooled_prompt_embeds_list, axis=-1)

    t5_prompt_embed = _encode_prompt_with_t5(
        text_encoders[-1],
        all_text_input_ids[-1],
        num_images_per_prompt=num_images_per_prompt,
    )
...
def main():
    ...
    if not train_dataset.custom_instance_prompts:
        ...
        if not args.train_text_encoder:
            all_text_input_ids = (tokens_one, tokens_two, tokens_three)
            prompt_embeds, pooled_prompt_embeds = encode_prompt(text_encoders, all_text_input_ids)
        else:
    ...
    text_encoder_three = unwrap_model(text_encoder_three, prefix="text_encoder_three")

    # Optimization parameters
    transformer_parameters_with_lr = {"params": transformer.get_parameters(), "lr": args.learning_rate}
    if args.train_text_encoder:
        # different learning rate for text encoder and unet
        text_parameters_one_with_lr = {
            "params": text_encoder_one.get_parameters(),
            "weight_decay": args.adam_weight_decay_text_encoder,
            "lr": args.text_encoder_lr if args.text_encoder_lr else args.learning_rate,
        }
        text_parameters_two_with_lr = {
            "params": text_encoder_two.get_parameters(),
            "weight_decay": args.adam_weight_decay_text_encoder,
            "lr": args.text_encoder_lr if args.text_encoder_lr else args.learning_rate,
        }
        text_parameters_three_with_lr = {
            "params": text_encoder_three.get_parameters(),
            "weight_decay": args.adam_weight_decay_text_encoder,
            "lr": args.text_encoder_lr if args.text_encoder_lr else args.learning_rate,
        }
        params_to_optimize = [
...
class TrainStepForSD3DB(TrainStep):
    ...
    def compute_weighting_mse_loss(self, weighting, pred, target):
        """
        When self.args.with_prior_preservation is True, weighting has different batch_size with
        pred/target which causes errors, therefore we broadcast them to proper shape before mul
        """
        assert (
            weighting.ndim == 1
        ), f"weighting should be a 1-dim MindSpore Tensor with shape (bsz,), but got {weighting.shape}"

        target_ndim = target.ndim
        repeats = weighting.shape[0] // pred.shape[0]

        weighting = weighting.float().reshape((weighting.shape[0],) + (1,) * (target_ndim - 1))
    ...
    def forward(self, pixel_values, tokens_one, tokens_two, tokens_three):
        ...
        if not self.custom_instance_prompts:
            ...
            if not self.args.train_text_encoder:
                ...
                prompt_embeds, pooled_prompt_embeds = encode_prompt(
                    ...
                    all_text_input_ids=(self.tokens_one, self.tokens_two, self.tokens_three),
                )
        else:
            # encode batch prompts when custom prompts are provided for each image
            prompt_embeds, pooled_prompt_embeds = encode_prompt(
        ...
        elif self.args.weighting_scheme == "logit_normal":
            # See 3.1 in the SD3 paper ($rf/lognorm(0.00,1.00)$).
            u = ops.normal(mean=self.args.logit_mean, stddev=self.args.logit_std, shape=(bsz,))
            weighting = ops.sigmoid(u)
        elif self.args.weighting_scheme == "mode":
            # See sec 3.1 in the SD3 paper (20).
            u = ops.rand(bsz)
            weighting = 1 - u - self.args.mode_scale * (ops.cos(ms.numpy.pi * u / 2) ** 2 - 1 + u)
        else:
            raise ValueError(
                f"Argument weighting_scheme should be in ['sigma_sqrt', 'logit_normal', 'mode'], but got '{self.args.weighting_scheme}'"
            )
        ...
        if self.args.with_prior_preservation:
            ...
            # Compute prior loss
            prior_loss = self.compute_weighting_mse_loss(weighting, model_pred_prior, target_prior)
        else:
            prior_loss = None

        # Compute regular loss.
        loss = self.compute_weighting_mse_loss(weighting, model_pred, target)
