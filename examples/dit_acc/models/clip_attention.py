...
def clip_attention_construct(
    ...
    if causal_attention_mask is not None:
        if causal_attention_mask.shape != (bsz, 1, q_len, src_len):
            raise ValueError(
                f"Attention mask should be of size {(bsz, 1, q_len, src_len)}, but is" f" {causal_attention_mask.shape}"
            )
    ...
    _, _, softmax_out, attn_output = FlashAttentionScore(
        self.num_heads, keep_prob=1.0, sparse_mode=0, scale_value=self.scale, input_layout="BSH"
