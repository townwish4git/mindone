...
from gm.modules.embedders.chinese_clip import BertConfig, BertModel, TeleBertModel
...
class FrozenOpenCLIPEmbedder2TeleOpenCLIP(AbstractEmbModel):
    """
    Uses the OpenCLIP transformer encoder for text
    """
...
class FrozenTeleCnCLIPEmbedder2(AbstractEmbModel):
    """Uses the Bert transformer encoder for text (from huggingface)"""
    ...
    def __init__(
        self,
        version="./chinese_clip/model_configs/chinese_clip_H14_mod.json",
        tokenizer_version="bert-base-chinese",
        max_length=77,
        freeze=True,
        layer="last",
        ...
        with open(version, "r") as f:
            version = json_load(f)
        extra_version = version.get("extra_config")
        version = version.get("text_config")

        assert layer in self.LAYERS
        if layer == "hidden":
            extra_version["output_hidden_states"] = True

        config = BertConfig(**version)
        extra_config = BertConfig(**extra_version)
        self.transformer = TeleBertModel(config, extra_config)
        self.tokenizer = BertTokenizer.from_pretrained(tokenizer_version, do_lower_case=True)
        ...
        if layer == "hidden":
            assert layer_idx is not None
            assert 0 <= abs(layer_idx) <= extra_config.num_hidden_layers
