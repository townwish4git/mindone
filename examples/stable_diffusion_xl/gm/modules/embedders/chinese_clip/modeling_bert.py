...
__all__ = ["BertModel", "TeleBertModel"]
...
class TeleBertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, extra_config):
        ...
        self.embeddings = BertEmbeddings(config)
        self.encoder = BertEncoder(config)
        self.extra_proj = nn.Dense(config.hidden_size, extra_config.hidden_size)
        self.extra_encoder = BertEncoder(extra_config)
        # self.pooler = BertPooler(config) if add_pooling_layer else None
    ...
    def construct(self, input_ids):
        ...
        embedding_output = self.embeddings(input_ids, position_ids=position_ids, token_type_ids=token_type_ids)
        encoder_outputs = self.encoder(embedding_output)
        encoder_outputs = self.extra_proj(encoder_outputs[0])
        extra_outputs = self.extra_encoder(encoder_outputs)

        sequence_output = extra_outputs[0]
        # pooled_output = (
        #     self.pooler(sequence_output) if self.pooler is not None else None
        # )
