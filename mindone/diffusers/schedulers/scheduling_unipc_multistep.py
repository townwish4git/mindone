        ...
        **kwargs,
        ...
        if len(D1s) > 0:
            ...
            if order == 2:
                ...
                rhos_p = ms.Tensor(np.linalg.solve(R[:-1, :-1].asnumpy(), b[:-1].asnumpy()))
        else:
            D1s = None
        ...
        if self.predict_x0:
            ...
            if D1s is not None:
                pred_res = ops.einsum("k,bkc...->bc...", rhos_p, D1s)
            else:
                pred_res = 0
    ...
    def multistep_uni_c_bh_update(
        ...
        **kwargs,
        ...
        if order == 1:
            ...
            rhos_c = ms.Tensor(np.linalg.solve(R.asnumpy(), b.asnumpy()))

        if self.predict_x0:
            x_t_ = sigma_t / sigma_s0 * x - alpha_t * h_phi_1 * m0
            if D1s is not None:
                corr_res = ops.einsum("k,bkc...->bc...", rhos_c[:-1], D1s)
            else:
                corr_res = 0
            D1_t = model_t - m0
