...
class PNDMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def _get_prev_sample(self, sample, timestep, prev_timestep, model_output):
        ...
        if self.config.prediction_type == "v_prediction":
            model_output = ((alpha_prod_t**0.5) * model_output).to(model_output.dtype) + ((beta_prod_t**0.5) * sample).to(sample.dtype)
        ...
        # full formula (9)
        prev_sample = (
            (sample_coeff).to(sample.dtype) * sample - ((alpha_prod_t_prev - alpha_prod_t).to(model_output.dtype) * model_output / model_output_denom_coeff).to(model_output.dtype)
        )
