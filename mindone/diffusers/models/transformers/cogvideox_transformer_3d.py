...
class CogVideoXTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        spatial_pos_embedding = ms.Tensor.from_numpy(spatial_pos_embedding).flatten(start_dim=0, end_dim=1)
        pos_embedding = ops.zeros(size=(1, max_text_seq_length + self.num_patches, inner_dim))
    ...
    def construct(
        ...
        if not self.config["use_rotary_positional_embeddings"]:
            ...
            pos_embeds = self.pos_embedding[:, : text_seq_length + seq_length].to(hidden_states.dtype)
