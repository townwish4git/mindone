...
class UNetControlNetXSModel(ModelMixin, ConfigMixin):
    ...
    def from_unet(
        ...
        # create model and cast dtype to ensure conbined model has the same dtype as the unet and controlnet
        # and avoid tons of warning when MindSpore load parameters in state dict to those in model
    ...
    def construct(
        ...
        ctrl_temb, base_temb, interpolation_param = (None,) * 3
        ...
        time_ids, time_embeds, add_embeds, aug_emb = (None,) * 4
...
def sync_dtype(module: nn.Cell, other: nn.Cell):
    ...
    This function is designed to prevent warnings arising from data type mismatches when later
    synchronizing parameters between two models. In MindSpore, such mismatches can lead to
    numerous warning messages, whereas PyTorch tends to handle these situations more silently.
    """
    _, first_p = next(other.parameters_and_names())
    target_dtype = first_p.dtype

    for p in module.get_parameters():
        p.set_dtype(target_dtype)
