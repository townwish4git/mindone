...
class DownBlockControlNetXSAdapter(nn.Cell):
...
class MidBlockControlNetXSAdapter(nn.Cell):
    """Components that together with corresponding components from the base model will form a
    `ControlNetXSCrossAttnMidBlock2D`"""

    def __init__(self, midblock: UNetMidBlock2DCrossAttn, base_to_ctrl: nn.CellList, ctrl_to_base: nn.CellList):
        super().__init__()
...
def get_down_block_adapter(
    ...
    down_block_components = DownBlockControlNetXSAdapter(
        resnets=nn.CellList(resnets),
        base_to_ctrl=nn.CellList(base_to_ctrl),
        ctrl_to_base=nn.CellList(ctrl_to_base),
    )

    if has_crossattn:
        down_block_components.attentions = nn.CellList(attentions)
...
def get_up_block_adapter(
    ...
    return UpBlockControlNetXSAdapter(ctrl_to_base=nn.CellList(ctrl_to_base))
...
class ControlNetXSAdapter(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # see https://github.com/huggingface/diffusers/issues/2011#issuecomment-1547958131 for why `ControlNetXSAdapter` takes `num_attention_heads` instead of `attention_head_dim`  # noqa: E501
        ...
        self.up_connections = nn.CellList(self.up_connections)
    ...
    def from_unet(
        ...
        if num_attention_heads is None:
            # The naming seems a bit confusing and it is, see https://github.com/huggingface/diffusers/issues/2011#issuecomment-1547958131 for why.  # noqa: E501
    ...
    def construct(self, *args, **kwargs):
        raise ValueError(
            "A ControlNetXSAdapter cannot be run by itself. Use it together with a UNet2DConditionModel to instantiate a UNetControlNetXSModel."  # noqa: E501
        )
...
class UNetControlNetXSModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if addition_embed_type is not None and addition_embed_type != "text_time":
            raise ValueError(
                "As `UNetControlNetXSModel` currently only supports StableDiffusion and StableDiffusion-XL, `addition_embed_type` must be `None` or `'text_time'`."  # noqa: E501
            )
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            up_blocks_resnets_lens.append(len(up_blocks[-1].resnets))
    ...
    def from_unet(
        ...
        if controlnet is None:
            ...
            if any(
                ...
                raise ValueError(
                    "When a controlnet is passed, none of these parameters should be passed: size_ratio, ctrl_block_out_channels, time_embedding_mix, ctrl_optional_kwargs."  # noqa: E501
                )
        ...
        for m in modules_from_unet:
            ms.load_param_into_net(getattr(model, "base_" + m), getattr(unet, m).parameters_dict())
    ...
    def construct(
        ...
        if self.config["addition_embed_type"] is None:
            ...
            raise ValueError(
                f"ControlNet-XS currently only supports StableDiffusion and StableDiffusion-XL, so addition_embed_type = {self.config['addition_embed_type']} is currently not supported."  # noqa: E501
            )
        ...
        for i, up in enumerate(self.up_blocks):
            n_resnets = self.up_blocks_resnets_lens[i]
...
class ControlNetXSCrossAttnDownBlock2D(nn.Cell):
    def __init__(
        self,
        base_in_channels: int,
        base_out_channels: int,
        ...
        self.base_resnets = nn.CellList(base_resnets)
        self.ctrl_resnets = nn.CellList(ctrl_resnets)
        self.base_attentions = nn.CellList(base_attentions) if has_crossattn else [None] * num_layers
        self.ctrl_attentions = nn.CellList(ctrl_attentions) if has_crossattn else [None] * num_layers
        self.base_to_ctrl = nn.CellList(base_to_ctrl)
        self.ctrl_to_base = nn.CellList(ctrl_to_base)
    ...
    def from_modules(cls, base_downblock: CrossAttnDownBlock2D, ctrl_downblock: DownBlockControlNetXSAdapter):
        ...
        temb_channels = base_downblock.resnets[0].time_emb_proj.in_channels
        ...
        # # load weights
        ms.load_param_into_net(model.base_resnets, base_downblock.resnets.parameters_dict())
        ms.load_param_into_net(model.ctrl_resnets, ctrl_downblock.resnets.parameters_dict())
        if has_crossattn:
            ms.load_param_into_net(model.base_attentions, base_downblock.attentions.parameters_dict())
            ms.load_param_into_net(model.ctrl_attentions, ctrl_downblock.attentions.parameters_dict())
        if add_downsample:
            ms.load_param_into_net(model.base_downsamplers, base_downblock.downsamplers[0].parameters_dict())
            ms.load_param_into_net(model.ctrl_downsamplers, ctrl_downblock.downsamplers.parameters_dict())
        ms.load_param_into_net(model.base_to_ctrl, ctrl_downblock.base_to_ctrl.parameters_dict())
        ms.load_param_into_net(model.ctrl_to_base, ctrl_downblock.ctrl_to_base.parameters_dict())
    ...
    def freeze_base_params(self) -> None:
        ...
        if isinstance(self.base_attentions, nn.CellList):  # attentions can be a list of Nones
    ...
    def construct(
        self,
        hidden_states_base: ms.Tensor,
        temb: ms.Tensor,
        encoder_hidden_states: Optional[ms.Tensor] = None,
        hidden_states_ctrl: Optional[ms.Tensor] = None,
        conditioning_scale: Optional[float] = 1.0,
        attention_mask: Optional[ms.Tensor] = None,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        encoder_attention_mask: Optional[ms.Tensor] = None,
        apply_control: bool = True,
    ) -> Tuple[ms.Tensor, ms.Tensor, Tuple[ms.Tensor, ...], Tuple[ms.Tensor, ...]]:
...
class ControlNetXSCrossAttnMidBlock2D(nn.Cell):
    def __init__(
        self,
        base_channels: int,
        ctrl_channels: int,
    ...
    def from_modules(
        ...
        temb_channels = base_midblock.resnets[0].time_emb_proj.in_channels
        ...
        # load weights
        ms.load_param_into_net(model.base_to_ctrl, base_to_ctrl.parameters_dict())
        ms.load_param_into_net(model.base_midblock, base_midblock.parameters_dict())
        ms.load_param_into_net(model.ctrl_midblock, ctrl_midblock.parameters_dict())
        ms.load_param_into_net(model.ctrl_to_base, ctrl_to_base.parameters_dict())
    ...
    def freeze_base_params(self) -> None:
        ...
        # Freeze base part
        for param in self.base_midblock.get_parameters():
            param.requires_grad = False
...
class ControlNetXSCrossAttnUpBlock2D(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        self.attentions = nn.CellList(attentions) if has_crossattn else [None] * num_layers
        self.ctrl_to_base = nn.CellList(ctrl_to_base)
    ...
    def from_modules(cls, base_upblock: CrossAttnUpBlock2D, ctrl_upblock: UpBlockControlNetXSAdapter):
        ...
        temb_channels = base_upblock.resnets[0].time_emb_proj.in_channels
        ...
        # load weights
        ms.load_param_into_net(model.resnets, base_upblock.resnets.parameters_dict())
        if has_crossattn:
            ms.load_param_into_net(model.attentions, base_upblock.attentions.parameters_dict())
        if add_upsample:
            ms.load_param_into_net(model.upsamplers, base_upblock.upsamplers[0].parameters_dict())
        ms.load_param_into_net(model.ctrl_to_base, ctrl_to_base_skip_connections.parameters_dict())
    ...
    def freeze_base_params(self) -> None:
        ...
        if isinstance(self.attentions, nn.CellList):  # attentions can be a list of Nones
    ...
    def construct(
        ...
        res_hidden_states_tuple_base: Tuple[ms.Tensor, ...],
        res_hidden_states_tuple_ctrl: Tuple[ms.Tensor, ...],
        temb: ms.Tensor,
        encoder_hidden_states: Optional[ms.Tensor] = None,
        conditioning_scale: Optional[float] = 1.0,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        ...
        for i in range(self.num_layers):
            # reversed operation not fully supported in GRAPH MODE, use index to get block instead.
            resnet, attn, c2b, res_h_base, res_h_ctrl = (
                self.resnets[i],
                self.attentions[i],
                self.ctrl_to_base[i],
                res_hidden_states_tuple_base[self.num_layers - i - 1],
                res_hidden_states_tuple_ctrl[self.num_layers - i - 1],
            )
            if apply_control:
