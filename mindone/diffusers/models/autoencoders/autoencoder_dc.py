...
from ..layers_compat import interpolate
...
class DCUpBlock2d(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        if self.interpolate:
            x = interpolate(hidden_states, scale_factor=float(self.factor), mode=self.interpolation_mode)
            x = self.conv(x)
        else:
            x = self.conv(hidden_states)
...
class Encoder(nn.Cell):
    def __init__(
        ...
        for i, (out_channel, num_layers) in enumerate(zip(block_out_channels, layers_per_block)):
            ...
            down_blocks.append(nn.SequentialCell(*down_block_list))
...
class AutoencoderDC(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def encode(self, x: ms.Tensor, return_dict: bool = False) -> Union[EncoderOutput, Tuple[ms.Tensor]]:
        r"""
