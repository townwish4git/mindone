...
class CogVideoXCausalConv3d(nn.Cell):
    ...
    def _clear_fake_context_parallel_cache(self):
        # del self.conv_cache  // comment it because it is not supported in GRAPH MODE
        self.conv_cache = None
...
class CogVideoXEncoder3D(nn.Cell):
    ...
    def construct(self, sample: ms.Tensor, temb: Optional[ms.Tensor] = None) -> ms.Tensor:
...
class AutoencoderKLCogVideoX(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def _clear_fake_context_parallel_cache(self):
        def fn_recursive_clear_fake_context_parallel_cache(module: nn.Cell):
            if isinstance(module, CogVideoXCausalConv3d):
                module._clear_fake_context_parallel_cache()

            for _, child in module.name_cells().items():
                fn_recursive_clear_fake_context_parallel_cache(child)

        for _, module in self.name_cells().items():
            fn_recursive_clear_fake_context_parallel_cache(module)
