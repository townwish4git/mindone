...
class Attention(nn.Cell):
    ...
    def fuse_projections(self, fuse=True):
        ...
        if hasattr(self, "add_q_proj") and hasattr(self, "add_k_proj") and hasattr(self, "add_v_proj"):
            ...
            self.to_added_qkv = nn.Dense(in_features, out_features, bias=self.added_proj_bias, dtype=dtype)
            self.to_added_qkv.weight = concatenated_weights
            if self.added_proj_bias:
                concatenated_bias = ops.cat([self.add_q_proj.bias, self.add_k_proj.bias, self.add_v_proj.bias])
                self.to_added_qkv.bias = concatenated_bias
