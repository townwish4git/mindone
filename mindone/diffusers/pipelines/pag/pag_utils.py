...
class PAGMixin:
    ...
    def _set_pag_attn_processor(self, pag_applied_layers, do_classifier_free_guidance):
        ...
        if pag_attn_processors is None:
            raise ValueError(
                "No PAG attention processors have been set. "
                "Set the attention processors by calling `set_pag_applied_layers` and passing the relevant parameters."
            )

        pag_attn_proc = pag_attn_processors[0] if do_classifier_free_guidance else pag_attn_processors[1]

        if hasattr(self, "unet"):
            model: nn.Cell = self.unet
        else:
            model: nn.Cell = self.transformer

        def is_self_attn(module: nn.Cell) -> bool:
            r"""
    ...
    def _apply_perturbed_attention_guidance(self, noise_pred, do_classifier_free_guidance, guidance_scale, t):
        ...
        Args:
            noise_pred (ms.Tensor): The noise prediction tensor.
    ...
    def _prepare_perturbed_attention_guidance(self, cond, uncond, do_classifier_free_guidance):
        ...
        Args:
            cond (ms.Tensor): The conditional input tensor.
            uncond (ms.Tensor): The unconditional input tensor.
