...
@ddt
class HunyuanSkyreelsImageToVideoPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_config = [
        [
            "transformer",
    ...
    def test_inference(self, mode, dtype):
        if dtype == "float32":
            pytest.skip(" FP32 is not supported since HunyuanSkyreelsImageToVideoPipeline contains nn.Conv3d")

        ms.set_context(mode=mode)

        pt_components, ms_components = self.get_dummy_components()
        pt_pipe_cls = get_module("diffusers.pipelines.hunyuan_video.HunyuanSkyreelsImageToVideoPipeline")
        ms_pipe_cls = get_module("mindone.diffusers.pipelines.hunyuan_video.HunyuanSkyreelsImageToVideoPipeline")
