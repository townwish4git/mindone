...
class SDSingleFileTesterMixin:
    def _compare_component_configs(self, pipe, single_file_pipe):
        for param_name, param_value in single_file_pipe.text_encoder.config.to_dict().items():
            if param_name in ["mindspore_dtype", "architectures", "_name_or_path"]:
                continue
    ...
    def test_single_file_components(self, pipe=None, single_file_pipe=None):
        single_file_pipe = single_file_pipe or self.pipeline_class.from_single_file(self.ckpt_path, safety_checker=None)
    ...
    def test_single_file_format_inference_is_same_as_pretrained(self, expected_max_diff=1e-4):
        ...
        image_single_file = sf_pipe(**inputs)[0][0]
    ...
    def test_single_file_setting_pipeline_dtype_to_fp16(
        ...
        single_file_pipe = single_file_pipe or self.pipeline_class.from_single_file(
            self.ckpt_path, mindspore_dtype=ms.float16
        )

        for component_name, component in single_file_pipe.components.items():
            if not isinstance(component, nn.Cell):
                continue

            assert next(component.get_parameters()).dtype == ms.float16
...
class SDXLSingleFileTesterMixin:
    ...
    def test_single_file_format_inference_is_same_as_pretrained(self, expected_max_diff=1e-4):
        sf_pipe = self.pipeline_class.from_single_file(self.ckpt_path, mindspore_dtype=ms.float16, safety_checker=None)
        sf_pipe.unet.set_default_attn_processor()

        inputs = self.get_inputs()
        image_single_file = sf_pipe(**inputs)[0][0]

        pipe = self.pipeline_class.from_pretrained(self.repo_id, mindspore_dtype=ms.float16, safety_checker=None)
