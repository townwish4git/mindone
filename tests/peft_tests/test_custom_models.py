...
from mindone.peft.utils import AuxiliaryTrainingWrapper
...
class MLPWithGRU(nn.Cell):
    def __init__(self, bias=True):
        super().__init__()
        self.lin0 = mint.nn.Linear(10, 20, bias=bias)
        self.relu = mint.nn.ReLU()
        self.drop = mint.nn.Dropout(0.5)
        self.gru = nn.GRU(input_size=20, hidden_size=20, num_layers=1, batch_first=True, has_bias=bias)
        self.fc = mint.nn.Linear(20, 2, bias=bias)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class MLP_LayerNorm(nn.Cell):
    def __init__(self, bias=True):
        super().__init__()
        self.layernorm0 = mint.nn.LayerNorm(10, 10)
        self.lin0 = mint.nn.Linear(10, 20, bias=bias)
        self.relu = mint.nn.ReLU()
        self.drop = mint.nn.Dropout(0.5)
        self.layernorm1 = mint.nn.LayerNorm(20, 20)
        self.lin1 = mint.nn.Linear(20, 2, bias=bias)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class MLP2(nn.Cell):
    def __init__(self, bias=True):
        super().__init__()
        self.lin0 = mint.nn.Linear(10, 32, bias=bias)
        self.relu = mint.nn.ReLU()
        self.drop = mint.nn.Dropout(0.5)
        self.lin1 = mint.nn.Linear(32, 2, bias=bias)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class Block(nn.Cell):
    def __init__(self, bias=True):
        ...
        self.relu = mint.nn.ReLU()
        self.drop = mint.nn.Dropout(0.5)
        self.lin1 = mint.nn.Linear(20, 10, bias=bias)
...
class DeepMLP(nn.Cell):
    def __init__(self, bias=True, num_hidden_layers=12):
        super().__init__()
        self.layers = nn.CellList([Block(bias=bias) for _ in range(num_hidden_layers)])
        self.out = mint.nn.Linear(10, 2, bias=bias)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class ModelEmbConv1D(nn.Cell):
    def __init__(self, emb_size=100):
        super().__init__()
        self.emb = mint.nn.Embedding(emb_size, 5)
        self.conv1d = Conv1D(1, 5)
        self.relu = mint.nn.ReLU()
        self.flat = mint.nn.Flatten()
...
class ModelConv1D(nn.Cell):
    def __init__(self):
        super().__init__()
        self.conv1d = mint.nn.Conv1d(1, 1, 2)
        self.relu = mint.nn.ReLU()
        self.flat = mint.nn.Flatten()
        self.lin0 = mint.nn.Linear(9, 2)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class ModelConv2D(nn.Cell):
    def __init__(self):
        super().__init__()
        self.conv2d = mint.nn.Conv2d(5, 10, 3)
        self.relu = mint.nn.ReLU()
        self.flat = mint.nn.Flatten()
        self.lin0 = mint.nn.Linear(10, 2)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class ModelConv2D2(nn.Cell):
    def __init__(self):
        super().__init__()
        self.lin0 = mint.nn.Linear(10, 40)
        self.conv2d = mint.nn.Conv2d(8, 32, 3)
        self.relu = mint.nn.ReLU()
        self.flat = mint.nn.Flatten()
        self.lin1 = mint.nn.Linear(32, 2)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class ModelConv3D(nn.Cell):
    def __init__(self):
        super().__init__()
        self.conv3d = mint.nn.Conv3d(5, 10, 3)
        self.relu = mint.nn.ReLU()
        self.flat = mint.nn.Flatten()
        self.lin0 = mint.nn.Linear(10, 2)
        self.sm = mint.nn.LogSoftmax(dim=-1)
...
class MockTransformerWrapper:
    ...
    @classmethod
    def from_pretrained(cls, model_id, mindspore_dtype=None):
        ...
        if model_id == "MLP":
            return MLP().to(mindspore_dtype)

        if model_id == "EmbConv1D":
            return ModelEmbConv1D().to(mindspore_dtype)

        if model_id == "Conv1d":
            return ModelConv1D().to(mindspore_dtype)

        if model_id == "Conv2d":
            return ModelConv2D().to(mindspore_dtype)

        if model_id == "Conv3d":
            return ModelConv3D().to(mindspore_dtype)

        if model_id == "MLP_LayerNorm":
            return MLP_LayerNorm().to(mindspore_dtype)

        if model_id == "MLP2":
            return MLP2().to(mindspore_dtype)

        if model_id == "Conv2d2":
            return ModelConv2D2().to(mindspore_dtype)

        if model_id == "MHA":
            return ModelMha().to(mindspore_dtype)
...
class PeftCustomModelTester(unittest.TestCase, PeftCommonTester):
    ...
    def test_forward_output_finite(self, test_name, model_id, config_cls, config_kwargs):
        ...
        with ms._no_grad():
            output = model(**X)
        assert mint.isfinite(output).all()
    ...
    def test_only_params_are_updated(self, test_name, model_id, config_cls, config_kwargs):
        ...
        for _ in range(3):
            grad_fn = ms.grad(loss_fn, None, weights=model.trainable_params())
            grads = grad_fn(X)
            optimizer(grads)

        tol = 1e-4
        params_before = dict(model_before.parameters_and_names())
        params_after = dict(model.parameters_and_names())
        ...
        for name, param_before in params_before.items():
            ...
            if (prefix in name) or ("modules_to_save" in name) or ("token_adapter.trainable_tokens" in name):
                ...
                assert not mint.allclose(param_before, param_after, atol=tol, rtol=tol)
            else:
                assert mint.allclose(param_before, param_after, atol=tol, rtol=tol)
    ...
    def test_disable_adapters(self, test_name, model_id, config_cls, config_kwargs):
        X = self.prepare_inputs_for_testing()
        model = self.transformers_class.from_pretrained(model_id).set_train(False)
        ...
        assert mint.allclose(outputs_base, outputs_before)
        ...
        if self.torch_device == "cpu":
            ...
            rtol, atol = 1e-5, 1e-8
        assert not mint.allclose(outputs_before, outputs_after, rtol=rtol, atol=atol)
        assert mint.allclose(outputs_before, outputs_disabled)
        assert mint.allclose(outputs_after, outputs_enabled_after_disable)
    ...
    def test_disable_adapters_with_merging(self, test_name, model_id, config_cls, config_kwargs):
        ...
        assert not mint.allclose(outputs_before, outputs_after, atol=atol, rtol=rtol)

        if self.torch_device in ["mlu"] and model_id in conv_ids:
            atol, rtol = 1e-3, 1e-2  # MLU

        # unmerged or merged should make no difference
        assert mint.allclose(outputs_after, outputs_unmerged, atol=atol, rtol=rtol)

        # check that disabling adapters gives the same results as before training
        assert mint.allclose(outputs_before, outputs_disabled, atol=atol, rtol=rtol)

        # check that enabling + disabling adapters does not change the results
        assert mint.allclose(outputs_after, outputs_enabled_after_disable, atol=atol, rtol=rtol)
    ...
    def test_adapter_name_makes_no_difference(self, config0):
        ...
        model_cls = MLP
        input = mint.arange(90).reshape(9, 10)

        # base model
        torch.manual_seed(0)
        base_model = model_cls().set_train(False)
        ...
        base_model = model_cls().set_train(False)
        torch.manual_seed(0)
        peft_model_default = get_peft_model(base_model, config0, adapter_name="default").set_train(False)
        ...
        base_model = model_cls().set_train(False)
        torch.manual_seed(0)
        peft_model_custom1 = get_peft_model(base_model, config0, adapter_name="adapter").set_train(False)
        ...
        base_model = model_cls().set_train(False)
        torch.manual_seed(0)
        peft_model_custom2 = get_peft_model(base_model, config0, adapter_name="other-name").set_train(False)
        ...
        for k0, k1, k2 in zip(sd_default, sd_custom1, sd_custom2):
            assert mint.allclose(sd_default[k0], sd_custom1[k1])
            assert mint.allclose(sd_default[k0], sd_custom2[k2])

        assert not mint.allclose(output_base, output_default)
        assert not mint.allclose(output_base, output_custom1)
        assert not mint.allclose(output_base, output_custom2)
        assert mint.allclose(output_custom1, output_custom2)
        assert mint.allclose(output_default, output_custom1)
    ...
    def test_dora_save_and_load_remapping(self):
        ...
        for k in state_dict:
            assert mint.allclose(state_dict[k], state_dict_loaded[k])
...
class MultipleActiveAdaptersTester(unittest.TestCase):
    ...
    def test_multiple_active_adapters_forward(
        ...
        assert not mint.allclose(adapter_1_output, adapter_2_output, atol=1e-5)
        assert not mint.allclose(adapter_1_output, combined_output, atol=1e-5)
        assert not mint.allclose(adapter_2_output, combined_output, atol=1e-5)
        ...
        if tuner_method == "lora":
            ...
            assert mint.allclose(new_combined_output, combined_output, atol=1e-5)
    ...
    def test_multiple_active_adapters_merge_and_unmerge(
        ...
        assert mint.allclose(merged_combined_output, combined_output, atol=1e-4)
        ...
        assert mint.allclose(disabled_adapter_output, base_output, atol=1e-4)
    ...
    def test_requires_grad_vera_different_targets(self):
        ...
        class MLP2(nn.Cell):
            def __init__(self, bias=True):
                ...
                self.relu = mint.nn.ReLU()
                self.lin0 = mint.nn.Linear(10, 20, bias=bias)
                self.lin1 = mint.nn.Linear(20, 20, bias=bias)  # lin1 and lin2 have same shape
                self.lin2 = mint.nn.Linear(20, 20, bias=bias)
                self.lin3 = mint.nn.Linear(20, 2, bias=bias)
                self.sm = mint.nn.LogSoftmax(dim=-1)
...
class TestMixedAdapterBatches:
    ...
    def mlp_lora(self):
        ...
        base_model = MLP().set_train(False)
        ...
        peft_model = get_peft_model(base_model, config0, "adapter0").set_train(False)
        peft_model.add_adapter("adapter1", config1)
        return peft_model
    ...
    def run_checks(self, model, inputs):
        ...
        assert not mint.allclose(output_base, output0)
        ...
        assert not mint.allclose(output_base, output0)
        assert not mint.allclose(output_base, output1)
        ...
        assert mint.allclose(output_base[::3], output_mixed[::3])
        assert mint.allclose(output0[1::3], output_mixed[1::3])
        assert mint.allclose(output1[2::3], output_mixed[2::3])

    def test_mixed_adapter_batches_lora_mlp(self, mlp_lora):
        inputs = {"X": mint.arange(90).view(-1, 10)}
    ...
    def test_mixed_adapter_batches_lora_unsupported_layer_raises(self, mlp_lora):
        base_model = MLPWithGRU().set_train(False)
    ...
    def test_mixed_adapter_batches_lora_conv1d_emb(self):
        base_model = ModelEmbConv1D().set_train(False)
    ...
    def test_mixed_adapter_batches_lora_conv2d(self):
        base_model = ModelConv2D().set_train(False)
        ...
        inputs = {"X": mint.arange(270).view(6, 5, 3, 3)}
        self.run_checks(peft_model, inputs)

    def test_mixed_adapter_batches_mha_raises(self):
        base_model = ModelMha().set_train(False)
    ...
    def test_mixed_adapter_batches_lora_training_mode_raises(self, mlp_lora):
        ...
        mlp_lora = mlp_lora.set_train(True)
    ...
    def test_mixed_adapter_batches_lora_disabled(self, mlp_lora):
        ...
        assert mint.allclose(output_disabled, output_mixed)
    ...
    def test_mixed_adapter_batches_lora_wrong_adapter_name_raises(self):
        ...
        peft_model = get_peft_model(base_model, config).set_train(False)
    ...
    def test_mixed_adapter_batches_lora_opt_timing(self):
        ...
        base_model = AutoModelForCausalLM.from_pretrained("facebook/opt-125m").set_train(False)
        inputs = {"input_ids": torch.randint(0, 1000, (16, 64))}
        with timed():
            output_base = base_model(**inputs).logits

        config0 = LoraConfig(task_type="CAUSAL_LM", init_lora_weights=False)
        peft_model = get_peft_model(base_model, config0, "adapter1").set_train(False)
        with timed():
        ...
        atol, rtol = 1e-4, 1e-4
        assert mint.allclose(output_base[::3], output_mixed[::3], atol=atol, rtol=rtol)
        assert mint.allclose(output0[1::3], output_mixed[1::3], atol=atol, rtol=rtol)
        assert mint.allclose(output1[2::3], output_mixed[2::3], atol=atol, rtol=rtol)
...
class TestDynamicDispatch:
    ...
    def test_saving_and_loading(self, custom_module_cls, custom_lora_cls, model_cls, tmp_path):
        ...
        assert mint.allclose(outputs_before, outputs_after)

        sd_after = peft_model.state_dict()
        assert sd_before.keys() == sd_after.keys()
        for key in sd_before.keys():
            assert mint.allclose(sd_before[key], sd_after[key])
    ...
    def test_target_layer_without_in_features_out_features(self, recwarn):
        ...
        class MyLora(nn.Cell, lora.LoraLayer):
