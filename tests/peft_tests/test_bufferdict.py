...
from mindone.peft.tuners._buffer_dict import BufferDict
...
class TestBufferDict:
    def test_init_from_dict_works(self):
        bd = BufferDict(
            {
                "default": mint.randn(10, 2),
            }
        )

    def test_update_from_other_bufferdict(self):
        default_tensor = mint.randn(10, 2)
        non_default_tensor = mint.randn(10, 2)
        ...
        assert mint.allclose(bd1["default"], default_tensor)
        assert mint.allclose(bd1["non_default"], non_default_tensor)
