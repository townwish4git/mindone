...
from townwish_mindone_testing.safetensors.mindspore import load_file as safe_load_file

from .constants import EMBEDDING_LAYER_NAMES, SAFETENSORS_WEIGHTS_NAME, WEIGHTS_NAME
...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
    ...
    with silence_mindspore_logger():
        param_not_load, ckpt_not_load = ms.load_param_into_net(model, peft_model_state_dict, strict_load=True)
    ...
    if config.peft_type == PeftType.MULTITASK_PROMPT_TUNING:
        ms.load_param_into_net(model.prompt_encoder[adapter_name], peft_model_state_dict, strict_load=True)
    return load_result


def load_peft_weights(model_id: str, **hf_hub_download_kwargs) -> dict:
    r"""
