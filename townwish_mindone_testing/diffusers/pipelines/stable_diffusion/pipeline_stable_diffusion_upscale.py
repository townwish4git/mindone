...
class StableDiffusionUpscalePipeline(
    ...
    def check_inputs(
        ...
        if (
            ...
            raise ValueError(
                f"`image` has to be of type `ms.Tensor`, `np.ndarray`, `PIL.Image.Image` or `list` but is {type(image)}"
            )
    ...
    def __call__(
        ...
        if not output_type == "latent":
            ...
            image, has_nsfw_concept, _ = self.run_safety_checker(image, prompt_embeds.dtype)
        else:
            image = latents
